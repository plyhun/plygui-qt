...
impl FrameInner for QtFrame {
	fn with_label(label: &str) -> Box<Frame> {
		let mut fr = Box::new(Member::with_inner(Control::with_inner(SingleContainer::with_inner(QtFrame {
                     base: common::QtControlBase::with_params(
		                     	QGroupBox::new(&QString::from_std_str(label)),
		                     	event_handler,
                             ),
...
impl SingleContainerInner for QtFrame {
    fn set_child(&mut self, _base: &mut MemberBase, mut child: Option<Box<controls::Control>>) -> Option<Box<controls::Control>> {
        ...
        if let Some(old) = child.as_mut() {
            unsafe { (self.base.widget.as_mut().layout().as_mut().unwrap().dynamic_cast_mut().unwrap() as &mut QStackedLayout).remove_widget(common::cast_control_to_qwidget_mut(old.as_mut())); }
        }
        if let Some(new) = self.child.as_mut() {
        	unsafe { (self.base.widget.as_mut().layout().as_mut().unwrap().dynamic_cast_mut().unwrap() as &mut QStackedLayout).add_widget(common::cast_control_to_qwidget_mut(new.as_mut()) as *mut QWidget); }
        } 
        child
	}
...
impl Drawable for QtFrame {
	fn draw(&mut self, _member: &mut MemberBase, _control: &mut ControlBase, coords: Option<(i32, i32)>) {
    	...
    	if let Some((x, y)) = self.base.coords {
    	    self.base.widget.as_mut().move_((x as i32, y as i32));
                    ...
                    layout::Size::WrapContent => {
                        ...
                        if let Some(ref mut child) =  self.child {
		                    let (cw, _, _) = child.measure(
		                    	cmp::max(0, parent_width as i32 - lm + 2) as u16, 
